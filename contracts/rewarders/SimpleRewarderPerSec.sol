...
contract SimpleRewarderPerSec is IRewarder, BoringOwnable, ReentrancyGuard {
    ...
    modifier onlyMCM() {
        require(msg.sender == address(MCM), "onlyMCM: only MasterChefMojito can call this function");
        _;
    }
    ...
    function onMojitoReward(address _user, uint256 _lpAmount) external override onlyMCM nonReentrant {
        updatePool();
    ...
    /// @notice payable function needed to receive KCS
